...
Run: `pnpm test` Expected: PASS（1 passed）。
...
Run: `pnpm test src/widget/card.test.ts` Expected: FAIL（`generateCard` が無い）。
...
Run: `pnpm test src/widget/card.test.ts` Expected: PASS。
...
Run: `pnpm test src/widget/card.test.ts` Expected: FAIL（`markNumber`/`judge` 未定義）。
...
const ALL_LINES: Line[] = [...[0, 1, 2, 3, 4].map(index => ({kind: "row" as const, index})), ...[0, 1, 2, 3, 4].map(index => ({kind: "col" as const, index})), {kind: "diag", index: 0}, {kind: "diag", index: 1}]
...
Run: `pnpm test src/widget/card.test.ts` Expected: PASS（全テスト）。
...
Run: `pnpm test src/widget/hydrate.test.ts` Expected: FAIL（モジュール無し）。
...
Run: `pnpm test src/widget/hydrate.test.ts` Expected: PASS。
...
Run: `pnpm test src/widget/draw.test.ts` Expected: FAIL（モジュール無し）。
...
Run: `pnpm test src/widget/draw.test.ts` Expected: PASS。
...
function render(): void {
	...
	document.getElementById("status")!.textContent = bingoLines.length ? "ビンゴ！" : reachLines.length ? `リーチ ${reachLines.length}` : ""
...
document.getElementById("draw")!.addEventListener("click", async () => {
	...
	if (n === null) {
		document.getElementById("status")!.textContent = "全て抽選済み"
		return
	}
...
Run: `pnpm build:widget` Expected: `wrote dist/mcp-app.html` が出力され、`dist/mcp-app.html` に `<script>` が含まれる。
...
Run: `pnpm build` Expected: `dist/server/index.js` と `dist/mcp-app.html` が生成される。
