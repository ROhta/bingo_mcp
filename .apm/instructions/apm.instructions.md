...
| `apm.yml`（`dependencies.mcp` / `dependencies.apm`）                                                                            | プラグイン参照とリポジトリ固有 MCP の SoT（人間が編集）。共通 MCP セットの SoT は apm-config/mcp-toolkit | ✅   |
...
生成物のうち `apm.lock.yaml` と `.github/instructions/{pr-review,language}.instructions.md`（base 由来・クラウド Copilot 経路確保の例外）以外は `.gitignore` 対象のためコミットに含まれない。
...
## MCP サーバー（共通セットは apm-config/mcp-toolkit）

共通の MCP サーバーセット（semgrep / context7 / serena / chrome-devtools）は共有パッケージ [`ROhta/apm-config/mcp-toolkit`](https://github.com/ROhta/apm-config) が SoT。`apm.yml` の `dependencies.apm` から参照し、`apm install` で直接依存として自動信頼・展開される。

| 名前              | 用途                                 | 前提   |
| ----------------- | ------------------------------------ | ------ |
| `semgrep`         | 静的解析（SAST）                     | `uv`   |
| `context7`        | ライブラリ公式ドキュメント最新版取得 | `node` |
| `serena`          | LSP ベースのシンボル指向コード探索   | `uv`   |
| `chrome-devtools` | ブラウザ自動化・widget 検証          | `node` |

- **ピン方針**: 各サーバーの pin（serena の SHA 等）は mcp-toolkit 側で一元管理。更新は apm-config を編集し `apm update` で取り込む（従来リポジトリ間で pin がドリフトしていた問題を解消）。
- リポジトリ固有 MCP が必要な場合のみ `apm.yml` の `dependencies.mcp` に個別追記する。
- **APM レジストリ（`apm mcp search` / `apm mcp install <registry-name>`）は使わない**。解決不正の既知問題があるため、すべて self-defined で登録する（mcp-toolkit 側も同方針）。
